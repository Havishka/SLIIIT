...
if(isset($_REQUEST['submit']))
{
	...
	if(($le==1))
	{
	$sql = " SELECT * FROM student s, payment p, course c, student_branch sb, branch b, student_course sc where s.std_id = sb.std_id and sb.branch_id=b.branch_id and sc.course_id=p.course_id and s.std_id=p.std_id and p.discount>0 and s.std_id=sc.std_id and sc.course_id=c.course_id and (s.nic LIKE '%$inputmy%' or s.fname LIKE '%$inputmy%' or s.lname LIKE '%$inputmy%') and p.type!='FullPay' and p.approvalState!='F' order by b.name asc , p.course_id  ";
	}
	if(($le==2))
	{
	$sql = " SELECT * FROM student s, payment p, course c, student_branch sb, branch b, student_course sc where s.std_id = sb.std_id and sb.branch_id=b.branch_id and sc.course_id=p.course_id and s.std_id=p.std_id and p.discount>0 and s.std_id=sc.std_id and sc.course_id=c.course_id and (s.nic LIKE '%$inputmy%' or s.fname LIKE '%$inputmy%' or s.lname LIKE '%$inputmy%') and sb.branch_id='$bid' and p.type!='FullPay' and p.approvalState!='F' order by b.name asc , p.course_id  ";
	}
