...
while ($row12 = mysqli_fetch_array($rs3)) {
    ...
    $student_idstudent = $row12['student_idstudent'] == null ? '' : $row12['student_idstudent'];
}
...
$sql11 = "select price AS f_i_price from first_install_course where course_idcourse='$c_id'";
$sql12 = "select price AS s_i_price from second_install_course where course_idcourse='$c_id'";
$sql13 = "select price AS t_i_price from third_install_course where course_idcourse='$c_id'";

$sql20 = "SELECT SUM(amount) AS first_sum FROM `student_has_first_install` WHERE `student_idstudent`='$student_idstudent' && `course_idcourse`='$c_id'";
$sql21 = "SELECT SUM(amount) AS second_sum FROM `student_has_second_install` WHERE `student_idstudent`='$student_idstudent' && `course_idcourse`='$c_id'";
$sql22 = "SELECT SUM(amount) AS third_sum FROM `student_has_third_install` WHERE `student_idstudent`='$student_idstudent' && `course_idcourse`='$c_id'";
...
$rs12 = $conn->query($sql11);
$rs13 = $conn->query($sql12);
$rs14 = $conn->query($sql13);
            ...
            while ($row10 = mysqli_fetch_array($rs10)) {
                while ($row11 = mysqli_fetch_array($rs11)) {
                    ...
                    while ($row12 = mysqli_fetch_array($rs12)) {
                        while ($row13 = mysqli_fetch_array($rs13)) {
                            while ($row14 = mysqli_fetch_array($rs14)) {
                                ...
                                while ($row15 = mysqli_fetch_array($rs20)) {
                                    while ($row16 = mysqli_fetch_array($rs21)) {
                                        while ($row17 = mysqli_fetch_array($rs22)) {
                                            ...
                                            $f_i_price = $row12['f_i_price'] == null ? '' : $row12['f_i_price'];
                                            $s_i_price = $row13['s_i_price'] == null ? '' : $row13['s_i_price'];
                                            $t_i_price = $row14['t_i_price'] == null ? '' : $row14['t_i_price'];
                                            ...
                                            $all->f_i_price = $f_i_price;
                                            $all->s_i_price = $s_i_price;
                                            $all->t_i_price = $t_i_price;
