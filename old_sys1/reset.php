...
session_destroy();	
	...
	if (!isset($_SESSION['FINALisok']))
	{
					...
					}else if ((isset($_SESSION['token']))&& ($_SESSION['FINAL']=="false"))
					{
						...
						$frm='<h3 style="color:white; font-size:15px;">Your confirmation code sent. check your email spam folder.</h3><br>    <form method="post" action="" style="padding:10px;">
	...
	}else if (isset($_SESSION['FINALisok']))
	{
				  ...
				  <br>
					...
					<input type="text"  name="Npassword" placeholder="New password"  required />
					<br>
	...
	if(isset($_POST['usn'])){
       ...
       <img src="inside/Admin/M_ADMIN/users_profile_images/'.$row["profile"].'" class="img-circle" alt="User" width="80" style=" 
    height:90px;
    border-radius:50%;
    overflow:hidden;" > 
	
	<p style="color:white;">Hello '.$row["name"].' '.$_SESSION['token'].'</p>
...
mail($to,$subject,$message,$headers);
	...
	if(isset($_POST['tokensub'])){
		...
		$_SESSION['FINALisok']="true";
				  ...
				  <br>
					...
					<input type="text"  name="Npassword" placeholder="Username" value="New password" required />
					<br>
				 
					<input type="password" name="RNpassword"  placeholder="e-enter New password"  value="" required />
	...
	if(isset($_POST['finalize'])){
		...
		echo "<script>alert('dsdsdsds');</script>";
					...
					if ($con->query($sqlxc) === TRUE) {
						...
						echo '<script>window.location= "http://sidanmor.com";</script>';
						session_destroy();
						
					} else {
						//echo "Error updating record: " . $con->error;
						echo '<script>alert("Error updating record.Please contact system administator.");</script>';
						session_destroy();
					}
