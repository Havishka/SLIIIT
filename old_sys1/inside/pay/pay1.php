...
<script src="reg/jquery-2.1.4.js" type="text/javascript"></script>
...
//only show full payment and initial payment once
   ...
   $sql23 = "SELECT * FROM student s, payment p WHERE s.std_id=p.std_id and s.nic like '%".$nic."%' and p.course_id like '%".$course_id."%'";
...
//else
{
   $sql24 = "SELECT * FROM student s, payment p WHERE s.std_id=p.std_id and  p.type='SPC' and s.nic like '%".$nic."%' and p.course_id like '%".$course_id."%'";
   $result24 = $con->query( $sql24 );
   $num_results24 = $result24->num_rows;
   
   $sql27 = "SELECT * FROM student s, payment p WHERE s.std_id=p.std_id and  p.type='FullPay' and s.nic like '%".$nic."%' and p.course_id like '%".$course_id."%'";
   $result27 = $con->query( $sql27 );
   $num_results27 = $result27->num_rows;
   
   $sql29 = "SELECT * FROM student s, payment p WHERE s.std_id=p.std_id and  p.approvalState ='F' and s.nic like '%".$nic."%' and p.course_id like '%".$course_id."%'";
   ...
   $num_results29 = $result29->num_rows;
	...
	if(($num_results24 == 0) && ($num_results23 != 0) && ($num_results27 == 0) && ($tot_amountx > $mypayment))
	{
echo "<a href='index.php?tab=installment_payment&&nic=$nic&&course_id=$course_id'><i class='fa fa-star-half-full'></i> Pay a Installment<span class='text-muted'></span><br></a>";
	...
	//check if discount modified
	  $sql15 = "SELECT * FROM student s, payment p WHERE s.std_id=p.std_id and  p.approvalState ='TC' and s.nic like '%".$nic."%' and p.course_id like '%".$course_id."%'";
  ...
  $sql14 = "SELECT * FROM student s, payment p WHERE s.std_id=p.std_id and  p.approvalState ='TC' and p.type='FullPay' and s.nic like '%".$nic."%' and p.course_id like '%".$course_id."%'";
